...
namespace MotionverseSDK
{
    public class AnswerDriveUtils
    {
        ...
        public static IEnumerator RealAnswerMotion(string text)
        {
            using (UnityWebRequest webRequest = new(Config.AnswerPrivateUrl, "POST"))
            {
                AnswerMotionParams postData = new()
                {
                    text = text
                };
                ...
                HandleMotion(text, webRequest.downloadHandler.text);
                TextMotionEnd(text);
            }
        }
        //错误、结束都清掉这个text任务
        ...
        private static void CastTask(string text)
        {
            ...
            {
                m_curTask.Add(text);
                TaskManager.Instance.Create(RealAnswerMotion(text));
            }
        }

        private static void HandleMotion(string text, string handle)
        {
